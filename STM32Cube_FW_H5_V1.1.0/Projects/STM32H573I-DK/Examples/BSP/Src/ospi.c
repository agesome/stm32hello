...
/* Private functions ---------------------------------------------------------*/
/**
  * @brief  Test OSPI interfaces and transfer modes
  * @param  None
  * @retval None
  */
void Test_Ospi(void)
{
  ...
  UTIL_LCD_DisplayStringAt(0, 15, (uint8_t *)" Test OSPI  ", CENTER_MODE);
...
/**
  * @brief  Test OSPI Read/Write in STR Mode.
  * @param  None
  * @retval None
  */
static void OSPI_STR_Mode(void)
{
  ...
  if(BSP_OSPI_NOR_DeInit(0) != BSP_ERROR_NONE) Error_Handler();
  ...
  if(BSP_OSPI_NOR_Init(0, &Flash) != BSP_ERROR_NONE) Error_Handler();
  
  /* Erase chip prior to program/read operations *************************************/ 
  if(BSP_OSPI_NOR_Erase_Block(0, OPI_START_ADDRESS, BSP_OSPI_NOR_ERASE_CHIP) != BSP_ERROR_NONE) Error_Handler();
  
  for(j = 0; j < 2; j++)
  {
    if(BSP_OSPI_NOR_ConfigFlash(0, InterfaceMode[j], Flash.TransferRate) != BSP_ERROR_NONE)
    {
      UTIL_LCD_SetTextColor(UTIL_LCD_COLOR_RED);  
...
/**
  * @brief  Test OSPI Read/Write in STR memory Mapped Mode.
  * @param  None
  * @retval None
  */
static void OSPI_STR_MM_Mode(void)
{
  ...
  for(j = 0; j < 2; j++)
  {
    ...
    if(BSP_OSPI_NOR_EnableMemoryMappedMode(0) != BSP_ERROR_NONE) Error_Handler();
    
    OSPI_MMP_pointer = (uint8_t*)(0x90000000 + OPI_START_ADDRESS);
    for(i = 0; i < (temp_value/BUFFER_SIZE); i++)
    {
    ...
    if(BSP_OSPI_NOR_DisableMemoryMappedMode(0) != BSP_ERROR_NONE) Error_Handler();
...
/**
  * @brief  Test OSPI Read/Write in DTR Mode.
  * @param  None
  * @retval None
  */
static void OSPI_DTR_Mode(void)
...
/**
  * @brief  Test OSPI Read/Write in DTR memory Mapped Mode.
  * @param  None
  * @retval None
  */
static void OSPI_DTR_MM_Mode(void)
