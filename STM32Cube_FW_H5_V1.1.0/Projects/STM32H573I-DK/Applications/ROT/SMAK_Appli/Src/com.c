...
HAL_StatusTypeDef  COM_Init(void)
{
  ...
  */
  ...
  UartHandle.AdvancedInit.OverrunDisable = UART_ADVFEATURE_OVERRUN_ENABLE;
  status = HAL_UART_Init(&UartHandle);
  if (status == HAL_OK)
  {
    return HAL_UARTEx_EnableFifoMode(&UartHandle);
  }
  return status;
}
...
/**
  ...
  * @retval None
  */
__weak HAL_StatusTypeDef Ymodem_HeaderPktRxCpltCallback(psa_image_id_t imageId,  uint32_t uOffset, uint32_t uFileSize)
{
  UNUSED(imageId);
  UNUSED(uOffset);
  UNUSED(uFileSize);
...
/**
  ...
  * @retval None
  */
__weak HAL_StatusTypeDef Ymodem_DataPktRxCpltCallback(uint8_t *pData, psa_image_id_t imageId, uint32_t uOffset,
                                                      uint32_t uSize)
{
  UNUSED(pData);
  UNUSED(imageId);
